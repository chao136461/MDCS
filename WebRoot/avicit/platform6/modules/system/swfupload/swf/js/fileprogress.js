...
function FileProgress(file,fileListID,swfUploadInstance,selectHtml,userName) {
	...
	if(!document.getElementById(file.id)){
		...
		**/
			...
			td.innerHTML="<center><span id="+file.id+"_per>等待上传</span></center><span id="+file.id+"_bar class=uploadBar style='display:none'></span>";
			td = tr.insertCell(-1);td.className='uploadTD';td.align="center";td.width="10%";
			...
			var tspan = document.createElement("span");
			tspan1.appendChild(tspan);
			tspan.innerHTML="<span class='perbar' style='display:none'><span id="+file.id+"_bar class='uploadBar2'></span></span><span id="+file.id+"_per>等待上传</span>";
			
			var tspan = document.createElement("span");
			tspan1.appendChild(tspan);
			tspan.innerHTML="<span id="+file.id+"_del  class='link-red''>取消</span>";
			var delObject = document.getElementById(file.id+"_del");
			delObject.onclick = function () {
				swfUploadInstance.cancelUpload(file.id);
				var tr = document.getElementById(file.id);
...
genSecretSelect = function(fileId,secret_level){
	...
	if(typeof(fileSecret)!="undefined"){
		selStr = "<select id='secret_"+fileId+"' title='附件密级' name='secret_"+fileId+"' onchange='javascript:setFilePara(\""+fileId+"\",\"secret\",this)'>";
...
genfileCategory = function(fileId,file_Category){
	...
	if(typeof(fileCategory)!="undefined"){
		selStr = "<select id='category_"+fileId+"' title='附件类型' name='category_"+fileId+"' onchange='javascript:setFilePara(\""+fileId+"\",\"category\",this)'>";
