...
<script type="text/javascript">
	...
	function downLoadTemplater(){
		var b = $('#factoryinfo').combobox('getValue');
		if(b=='--请选择机型--'){
			$.messager.alert('温馨提示','请先选择机型之后再导出模板!');
		 	return ;
		}
		var url = 'platform/discussion_manage/majormanage/MajorManageController/operation/majorExport?typeId='+typeId;//服务器请求地址
		var ep = new exportData("xlsExport","xlsExport","",url);
		ep.excuteExport();
		...
		<div id="toolbarExcel" class="datagrid-toolbar">
			<table>
				<tr>
					...
					<td><label style="padding-left: 15px;">机型:</label><input id="factoryinfo" name="jixing" value="--请选择机型--"/></td>
		...
		<form id="uploadExcel" style="padding-top:5px;" enctype="multipart/form-data" method="post">
			<table width="100%" border="0" cellspacing="1">
				<tr>
					...
					<td><a class="easyui-linkbutton" iconCls="icon-upload" plain="false" name="upload" id="upload" onclick="uploadExcel();" href="javascript:void(0);">导入</a></td>
				</tr>
			</table>
		</form>
		<div style="display: none;">
			<table id="dgUser" class="easyui-datagrid" datapermission='sysUser' data-options="toolbar:'#toolbarUser'">
			</table>
		</div>			
</div>
<script type="text/javascript">
		...
		if(document.getElementById("impExcelFile").value != ''){
			if(checkfiletype('impExcelFile')){
				...
				$('#uploadExcel').form('submit', {
					url: 'platform/discussion_manage/majormanage/majorUtil/importMajor',
					success: function(r){
