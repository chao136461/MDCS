...
function getPath(){
	...
	/* 增加新的路径判断，主要是格尔网关有可能设置多层项目名     */
	var baseHref = $('base').attr('href');
	if(typeof(baseHref) != "undefined" && baseHref != '' ){
		if(baseHref.indexOf('http') == 0 || baseHref.indexOf('https') == 0){
			if(baseHref.indexOf('//') > -1){
				var bindex = baseHref.indexOf('/', baseHref.indexOf('//') + 2);
				var _path = baseHref.substring(bindex);
				if (_path == "/") {
					_path = "";
				}
				if(_path != ''){
