 ...
 * 
 */
function RelevancePerson(datagrid,searchD,url,form){
...
**/
RelevancePerson.prototype.init=function(){
	var _self = this;
 ...
 //添加
RelevancePerson.prototype.insert=function(){
	if(!this.endEdit()){
...
//保存功能
RelevancePerson.prototype.save=function(){
	if(!this.endEdit()){
...
//编辑
RelevancePerson.prototype.edit=function(){
	var temp = this._datagrid;
...
//删除
RelevancePerson.prototype.del=function(){
...
//结束当前编辑行
RelevancePerson.prototype.endEdit=function(){
...
//重载数据
RelevancePerson.prototype.reLoad=function(){
	this._datagrid.datagrid('load',{});
};

//打开查询框
RelevancePerson.prototype.openSearchForm =function(){
	this.searchDialog.dialog('open',true);
};
//去后台查询
RelevancePerson.prototype.searchData =function(){
...
//隐藏查询框
RelevancePerson.prototype.hideSearchForm =function(){
	this.searchDialog.dialog('close',true);
};
/*清空查询条件*/
RelevancePerson.prototype.clearData =function(){
	this.searchForm.find('input').val('');
	this.searchData();
};
RelevancePerson.prototype.formate=function(value){
	if(value){
		return new Date(value).format("yyyy-MM-dd");
	}
	return '';
};
RelevancePerson.prototype.formateDateTime=function(value){
	if(value){
		return new Date(value).format("yyyy-MM-dd hh:mm:ss");
	}
	return '';
};
RelevancePerson.prototype.searchDataBySfn =function(conditions){
    this._datagrid.datagrid('uncheckAll').datagrid('unselectAll').datagrid('clearSelections');
 ...
 * Excel导入
 */
RelevancePerson.prototype.importData = function() {
	var imp = new CommonDialog(
			"importData",
			"700",
			"400",
			'platform/excelImportController/excelimport/importPersonData/xlsx',
...
function closeImportData() {
	...
	$('#dgRelevancePerson').datagrid('load',{deptCode: deptCode});
	$('#dgRelevancePerson').datagrid('uncheckAll').datagrid('unselectAll').datagrid('clearSelections');
	$("#importData").dialog('close');
};


//Excel客户端导出
RelevancePerson.prototype.exportClientData = function() {
	var _self = this;
	$.messager.confirm('确认', '是否确认导出Excel文件?', function(r) {
		if (r) { // 封装参数

			var columnFieldsOptions = getGridColumnFieldsOptions('dgRelevancePerson');
			var dataGridFields = JSON.stringify(columnFieldsOptions[0]);
			// 获得datagrid中的数据，getChecked：导出选中的数据；getRows:导出所有行数据
			var rows = $('#dgRelevancePerson').datagrid('getChecked');
			...
			var url ="platform/discussion_manage/relevanceperson/RelevancePersonController/operation/exportClient";// 导出请求地址
