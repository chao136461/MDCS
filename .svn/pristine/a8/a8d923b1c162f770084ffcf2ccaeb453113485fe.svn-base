 ...
 * 
 */
function RelevancePersonBranch(datagrid,searchD,url,form){
...
**/
RelevancePersonBranch.prototype.init=function(){
	var _self = this;
	this._datagrid=$(this._datagridId).datagrid({
		/*url:this._url +'getRelevancePersonBranchsByPage.json',*/
		url:this._url +'getSysUserByPage.json',
		queryParams: {
			deptCode: deptCode,
		},
 ...
 //添加
RelevancePersonBranch.prototype.insert=function(){
	if(!this.endEdit()){
...
//保存功能
RelevancePersonBranch.prototype.save=function(){
	if(!this.endEdit()){
	...
	if(rows.length > 0){
		 ...
		 $.ajax({
			 ...
			 success : function(r){
				 if (r.flag == "success"){
					 _self._datagrid.datagrid('reload',{deptCode: deptCode});//刷新当前页
...
//编辑
RelevancePersonBranch.prototype.edit=function(){
	var temp = this._datagrid;
...
//删除
RelevancePersonBranch.prototype.del=function(){
...
//结束当前编辑行
RelevancePersonBranch.prototype.endEdit=function(){
...
//打开查询框
RelevancePersonBranch.prototype.openSearchForm =function(){
	this.searchDialog.dialog('open',true);
};
//去后台查询
RelevancePersonBranch.prototype.searchData =function(){
...
//隐藏查询框
RelevancePersonBranch.prototype.hideSearchForm =function(){
	this.searchDialog.dialog('close',true);
};
/*清空查询条件*/
RelevancePersonBranch.prototype.clearData =function(){
	this.searchForm.find('input').val('');
	this.searchData();
};
RelevancePersonBranch.prototype.formate=function(value){
	if(value){
		return new Date(value).format("yyyy-MM-dd");
	}
	return '';
};
RelevancePersonBranch.prototype.formateDateTime=function(value){
	if(value){
		return new Date(value).format("yyyy-MM-dd hh:mm:ss");
	}
	return '';
};
RelevancePersonBranch.prototype.searchDataBySfn =function(conditions){
    this._datagrid.datagrid('uncheckAll').datagrid('unselectAll').datagrid('clearSelections');
    this._datagrid.datagrid('load', {
		deptCode: deptCode});
};
 ...
 * Excel导入
 */
RelevancePersonBranch.prototype.importData = function() {
	var imp = new CommonDialog(
			"importData",
			"700",
			"400",
			'platform/excelImportController/excelimport/importPersonBranchData/xlsx',
...
function closeImportData() {
	$('#dgRelevancePersonBranch').datagrid('load',{deptCode: deptCode});
	$('#dgRelevancePersonBranch').datagrid('uncheckAll').datagrid('unselectAll').datagrid('clearSelections');
	$("#importData").dialog('close');
};

//Excel客户端导出
RelevancePersonBranch.prototype.exportClientData = function() {
	var _self = this;
	$.messager.confirm('确认', '是否确认导出Excel文件?', function(r) {
		if (r) { // 封装参数

			var columnFieldsOptions = getGridColumnFieldsOptions('dgRelevancePersonBranch');
			var dataGridFields = JSON.stringify(columnFieldsOptions[0]);
			// 获得datagrid中的数据，getChecked：导出选中的数据；getRows:导出所有行数据
			var rows = $('#dgRelevancePersonBranch').datagrid('getChecked');
			...
			var url ="platform/discussion_manage/relevancepersonbranch/RelevancePersonBranchController/operation/exportClient";// 导出请求地址
