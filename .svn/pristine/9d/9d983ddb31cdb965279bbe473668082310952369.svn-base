 ...
 * 
 */
function DisputeMemo(datagrid,url,searchD,form){
...
//初始化操作
DisputeMemo.prototype.init=function(){
	var _self = this;
	...
	this._datagrid=$(this._datagridId).datagrid({
		url:this.getUrl()+"getDisputeMemosByPage.json"
		});
};
//添加页面
DisputeMemo.prototype.insert=function(){
	this.nData = new CommonDialog("insert","790","500",this.getUrl()+'Add/null',"添加",false,true,false);
	this.nData.show();
};
//编辑页面
DisputeMemo.prototype.modify=function(){
...
//详细页
DisputeMemo.prototype.detail=function(id){
	this.nData = new CommonDialog("edit","790","500",this.getUrl()+'Detail/'+id,"详情",false,true,false);
	this.nData.show();
};
//保存功能
DisputeMemo.prototype.save=function(form,id){
	var _self = this;
	$.ajax({
...
//删除
DisputeMemo.prototype.del=function(){
	var rows = this._datagrid.datagrid('getChecked');
	var _self = this;
	var ids = [];
	var l =rows.length;
...
//重载数据
DisputeMemo.prototype.reLoad=function(){
	this._datagrid.datagrid('load',{ param : JSON.stringify(serializeObject(this.searchForm))});
};
//关闭对话框
DisputeMemo.prototype.closeDialog=function(id){
	$(id).dialog('close');
};

//打开查询框
DisputeMemo.prototype.openSearchForm =function(){
	this.searchDialog.dialog('open',true);
};
//去后台查询
DisputeMemo.prototype.searchData =function(){
	this._datagrid.datagrid('uncheckAll').datagrid('unselectAll').datagrid('clearSelections');
	this._datagrid.datagrid('load',{ param : JSON.stringify(serializeObject(this.searchForm))});
};
//隐藏查询框
DisputeMemo.prototype.hideSearchForm =function(){
	this.searchDialog.dialog('close',true);
};
/*清空查询条件*/
DisputeMemo.prototype.clearData =function(){
	this.searchForm.find('input').val('');
	this.searchData();
};
DisputeMemo.prototype.formate=function(value){
	if(value){
		return new Date(value).format("yyyy-MM-dd");
	}
	return '';
};
DisputeMemo.prototype.formateDateTime=function(value){
	if(value){
		return new Date(value).format("yyyy-MM-dd hh:mm:ss");
	}
	return '';
};
DisputeMemo.prototype.searchDataBySfn =function(conditions){
    this._datagrid.datagrid('uncheckAll').datagrid('unselectAll').datagrid('clearSelections');
