 ...
 * 
 */
function MajorManage(datagrid,url,searchD,form,strId){
...
//初始化操作
MajorManage.prototype.init=function(){
...
/*	alert(_self._strId);*/
	...
	this._datagrid=$(this._datagridId).datagrid({
		...
		url:this.getUrl()+"getMajorManagesByPage.json",
		queryParams: {
...
//详细页
MajorManage.prototype.detail=function(id){
	this.nData = new CommonDialog("edit","790","500",this.getUrl()+'Detail/'+id,"详情",false,true,false);
	this.nData.show();
};
//保存功能
MajorManage.prototype.save=function(form,id){
	var _self = this;
	$.ajax({
...
//删除
MajorManage.prototype.del=function(){
	var rows = this._datagrid.datagrid('getChecked');
	var _self = this;
	var ids = [];
	var l =rows.length;
...
//重载数据
MajorManage.prototype.reLoad=function(){
	var _self = this;
...
//关闭对话框
MajorManage.prototype.closeDialog=function(id){
	$(id).dialog('close');
};

//打开查询框
MajorManage.prototype.openSearchForm =function(){
	this.searchDialog.dialog('open',true);
};
//去后台查询
MajorManage.prototype.searchData =function(){
...
//隐藏查询框
MajorManage.prototype.hideSearchForm =function(){
	this.searchDialog.dialog('close',true);
};
/*清空查询条件*/
MajorManage.prototype.clearData =function(){
	this.searchForm.find('input').val('');
	this.searchData();
};
MajorManage.prototype.formate=function(value){
	if(value){
		return new Date(value).format("yyyy-MM-dd");
	}
	return '';
};
MajorManage.prototype.formateDateTime=function(value){
	if(value){
...
//返回主表的id
MajorManage.prototype.getSelectID=function(){
	var rows = this._datagrid.datagrid('getChecked');
...
MajorManage.prototype.searchDataBySfn =function(conditions){
    this._datagrid.datagrid('uncheckAll').datagrid('unselectAll').datagrid('clearSelections');
